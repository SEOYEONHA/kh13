...
public class KakaoPayService {
	...
	public KakaoPayReadyResponseVO ready(KakaoPayReadyRequestVO requestVO) throws URISyntaxException {
		...
		String page = ServletUriComponentsBuilder.fromCurrentRequestUri().build().toUriString();
	...
	//여러 번의 등록 과정이 모두 성공하거나 모두 실패해야한다
	// -> 하나의 트랜잭션(transaction)으로 관리되어야한다
	...
	public void insertPayment(PurchaseListVO vo, KakaoPayApproveResponseVO responseVO) {
		//DB에 결제 완료된 내역을 저장
				...
				PaymentDto paymentDto = PaymentDto.builder()
													...
													.paymentRemain(responseVO.getAmount().getTotal()) //잔여금액(결제 총금액과 동일)
